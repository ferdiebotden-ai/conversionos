...
**Sample data:** `scripts/seed-demo-data.mjs` populates the demo tenant (`site_id='demo'`) with 2 fully-fleshed leads designed to showcase every tab and feature: (1) Sarah Mitchell — kitchen, premium, `draft_ready` status, 10-message Emma chat session, 1 uploaded photo, 4 AI-generated concept images, 8-item quote (~$64k total), voice dictation intake data; (2) Robert Kowalski — basement, premium, `won` status, 12-message Emma chat, 1 before photo, 4 concept images, 10-item quote (~$92k, sent+accepted), deposit invoice (paid), e-transfer payment. All images use local files from `public/images/` (Gemini-generated via `scripts/generate-image.mjs`, no external URL dependencies). Run `node scripts/seed-demo-data.mjs` after configuring `.env.local`. Idempotent — clears existing demo data before inserting.
