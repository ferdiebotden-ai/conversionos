 ...
 * If count < 5, creates drafts from the next demo_built targets.
 *
...
const builtRows = await query(
  "SELECT COUNT(*) as count FROM targets WHERE status = 'bespoke_ready' AND demo_url IS NOT NULL AND email_message_id IS NULL"
);
const builtCount = builtRows[0]?.count ?? 0;
...
logger.info(`Draft monitor: ${draftCount}/${TARGET_DRAFTS} drafts ready, ${builtCount} built (no draft), ${qualifiedCount} qualified (not yet built)`);
...
if (args.report) {
  if (draftCount >= TARGET_DRAFTS) {
    ...
    logger.info(`Need ${needed} more draft(s). Built targets available: ${builtCount}. Qualified targets available: ${qualifiedCount}.`);
  }
  process.exit(0);
}
...
if (builtCount === 0) {
  logger.warn(`Need ${needed} more draft(s) but no demo_built targets available. Run /maintain-pipeline or /build-tenant --batch to build more.`);
  process.exit(0);
}
