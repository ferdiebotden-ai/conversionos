# Unit 06: Seed Data — Update Existing Tenants with Extended Fields

## Scope
Create a new migration that updates the existing tenant seed data to include all the new `CompanyConfig` fields: `heroHeadline`, `heroSubheadline`, `heroImageUrl`, `aboutImageUrl`, `trustBadges`, `whyChooseUs`, `values`, `processSteps`, `teamMembers`, `portfolio`, `logoUrl`, and extended `services` with `slug`, `features`, and `packages`.
...
This migration UPSERTs the `company_profile` key for each tenant, adding the new fields while preserving existing data. Uses `jsonb_set` or full JSONB replacement (since UPSERT replaces the whole value anyway).

**IMPORTANT:** Use single quotes for SQL strings. Escape single quotes inside JSONB as `''`.

---

## McCarty Squared (`mccarty-squared`)

Based on what we know about McCarty Squared (London, ON renovation company, Garnet & Carisa, RenoMark certified):

```sql
-- Update McCarty Squared company_profile with extended fields
...
Also update `services` to include `slug`, `features`, and `packages`:
...
## Red White Reno (`redwhitereno`)

```sql
-- Update Red White Reno company_profile with extended fields
UPDATE admin_settings SET value = value || '{
...
-- Update Red White Reno services with slugs
UPDATE admin_settings SET value = jsonb_set(value, '{services}', '[
...
```sql
-- Update Demo company_profile with extended fields (generic fallback data)
UPDATE admin_settings SET value = value || '{
  "heroHeadline": "",
  "heroSubheadline": "",
  "heroImageUrl": "",
...
-- Update demo services with slugs
UPDATE admin_settings SET value = jsonb_set(value, '{services}', '[
...
Combine all the above into a single migration file: `supabase/migrations/20260222400000_extend_seed_data.sql`

Wrap in a transaction. Add a header comment explaining the purpose.
...
1. The SQL should be syntactically valid (no unmatched quotes)
2. Each UPDATE targets the correct `site_id` and `key`
3. JSONB concatenation (`||`) is used to ADD fields without overwriting existing ones
4. `jsonb_set` is used for the `services` array to REPLACE it entirely
5. All single quotes inside JSONB strings are properly escaped as `''`
